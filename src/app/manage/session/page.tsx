...
const Page: React.FC<SessionPageProps> = async (props) => {
  ...
  return (
    <div className="relative h-full">
      ...
      <section className="mb-4 w-full rounded-md bg-background/30 p-4 shadow-md">
        ...
        <div className="mb-4 flex items-center gap-4">
          ...
          <Link href={"session/create"}>
            <CreateButton text="Create session" extras="h-12" />
          </Link>
          <FilterButton />
          <ReloadButton />
        </div>
      ...
      <SessionTable pageIndex={pageIndex} pageSize={pageSize} term={term} />
    </div>
  );
};
